...
# ---------------------------------------------------------
# HARDWARE KEY MONITOR (Đọc phím cấp Kernel)
# ---------------------------------------------------------
...
class AgentLoopThread(threading.Thread):
    def __init__(self, ui_instance, eye_instance, audio_instance, brain_instance, key_monitor):
    ...
    def run(self):
        ...
        self.state = AgentState.SPEAKING
        self.ui.update_emotion_signal.emit("normal")
        self.audio.speak("Prototype 1 system is online. Awaiting manual override.")
        self.state = AgentState.IDLE
        
        print("\n" + "="*60)
        print(" PROTOTYPE_1 IS FULLY ONLINE. (Kernel-level monitoring active)")
        print(" PRESS [SPACEBAR] TO TALK. PRESS [ESC] TO QUIT UI.")
        print("="*60 + "\n")

        while self.is_running:
            if self.key_monitor.space_pressed:
                self.key_monitor.space_pressed = False 
    ...
    def _handle_user_interaction(self):
        ...
        user_text = self.audio.listen(seconds=5)
        if not user_text:
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("      Prototype_1 SYSTEM INITIALIZING (PROTOTYPE 1.0.2)     ")
    print("="*60)
    ...
    brain = AIBuddyBrain()
    key_monitor = HardwareKeyMonitor()
    
    agent_thread = AgentLoopThread(ui, eye, audio, brain, key_monitor)
